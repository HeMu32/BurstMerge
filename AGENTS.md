...
## 后端现状
	...
	- **Vulkan 后端** (已实现): 见下方 "Vulkan GPU 后端" 章节. 在 RTX 3080 / 5060 Ti 上验证, 全部合并模式与 CPU 近像素级一致 (等曝光 + 包围曝光 MAD < 0.02%; 包围曝光此前 < 0.25%, 引入 GPU 链式对齐 + EV 加权后降至 < 0.01%). 对齐 standard+dense bit-identical (max 0px); 包围曝光链式对齐 (transmission) 已在 GPU 实现. **默认全单精度 float; `dense_level.comp` v4 使用 64-thread-per-tile 协同 + 每 thread Kahan M=8 累加器 + 共享内存 tree-reduction (见下方 "GPU 已知性能陷阱" 段说明 v3→v4 演进); 可选 fp64** (`BURSTMERGE_GPU_FP64=ON`, 仅 `dense_level_fp64.comp` 的 `tileCostThreadFp64` 累加使用 double, 进一步提升极端包围曝光 dense 对齐精度; **fp64 v4 自 2026-07-05 起与 fp32 v4 同为 64-thread-per-tile 协同设计**, 朴素 double 累加无需 Kahan — double 52-bit mantissa 即使 t=128 ts²/64=256 个累加误差 < 5.7e-14, 与 CPU `align_common.cpp::TileCost` 的 `double cost += ...; return float(cost)` 精度等价).
...
==============================
	...
	- 对齐 (全 GPU, 零读回): 灰度金字塔 (2-then-4 模式) → 逐层 coarse-to-fine SAD 搜索 (sad_global 每 candidate 一个 workgroup, 256 线程 reduce; select_min 在 SSBO 内更新 seed, 无 CPU 读回; tie-breaking 匹配 CPU 字典序) → standard 路径 **2-pass 全并行 tile 细化** (tile_refine_diag: pass 0 全局 seed → pass 1 邻居传播 seed, dispatch `(tiles/8, tiles/8, 1)`; 非 CPU wavefront 的 bit-identical 复刻, 见 `docs/experiments/cpu-gpu-divergence.md` §4.5) 或 dense 路径逐层 propagate/correct/search (dense_level v4: **64-thread-per-tile 协同** — 每 workgroup 8×8 协同处理 1 tile, 共享 1KB shared memory 做 64-thread partial cost pairwise tree-reduction; ts² px 按 stride 64 平摊到 8 个 Kahan M=8 累加器; dispatch 维度 `(tiles_x, tiles_y, 1)` 替代 v3 的 `(ceil(tiles_x/8), ceil(tiles_y/8))`) → warp_tilefield 双线性混合 4 角 tile 位移. **[新增] 包围曝光链式对齐 (transmission)**: `ec.needs_chained_alignment` (spread > 2×) 时, `GpuPipelineCore` 按 EV 序顺序处理, 每帧对其到 EV 相邻邻居 (已 warp 进参考坐标系, 经 `to_grayscale` 重建灰度+金字塔), 而非直接对齐到参考帧. 与 CPU `BuildAlignedComparisons` 链式路径对应; 对齐 primitive (`align_to_parent` lambda) 固定参考/链式路径共用.
...
## 编译开关 (重要)
	...
	- `BURSTMERGE_GPU_FP64` (默认 **OFF**): 编译 GPU fp64 dense 对齐着色器 (`dense_level_fp64.comp`). 开启时 SPIR-V 含 36 个着色器, 关闭时 35 个. 需要运行时 GPU 支持 `shaderFloat64` (所有 NVIDIA/AMD 桌面 GPU 均支持). 运行时通过 `VulkanBackend::HasFloat64()` 检测, 不支持时自动回退到 float 着色器. **功能差异**: 默认 fp32 路径 `dense_level.comp` 已使用 strided M=16 累加缓解大部分分歧 (ExBkt t=32 max 74→46px); 开启 fp64 后进一步改善 (t=32 max →19px), 但在新 GPU (5060 Ti) 上代价较高 (+74~132% 对齐耗时 vs 默认 +37%). **性能**: fp64 dense 路径开销架构依赖 (3080: +4~74%; 5060 Ti: +74~132%); standard 路径不受影响. 构建命令: `cmake -S . -B build -DBURSTMERGE_GPU_FP64=ON`. **v4 移植 (2026-07-05)**: `dense_level_fp64.comp` 从 v3 (1-thread-per-tile) 改为 v4 (64-thread-per-tile), 朴素 double 累加 (无 Kahan, double 52-bit mantissa 精度足够), 与 fp32 v4 同 dispatch 形状, RTX 3080 实测 ExBkt t=128 bit-identical 到 CPU (max 0px), t=64 max 1 LSB / 3 / 6100 万 像素差异; LongUnder2 dense t=96 包围曝光 + 高光恢复 MAD 0.0037 优于 fp32 v4 的 0.022.
...
## GPU 已知性能陷阱 (2026-07-04 调查)
	- **SPIR-V 嵌入 build 系统已修复**: 历史上 `libburstmerge/CMakeLists.txt` 用 `execute_process` (configure-time) 嵌入 `spirv_embedded.inl`, 增量 `cmake --build` 不触发重新嵌入, 导致 `build_release/` 的 SPIR-V 是 6/22 旧版, 缺 6/30 提交的 `tile_refine_diag.comp` 2-pass 全并行优化. 在 SLg2-1-48 48 帧 + 5060 Ti 上为 standard alignment 表现为 1.1s/帧 (≈53s 总), CPU 仅 16.2s, 表面体现为 "GPU 比 CPU 慢 3 倍". 改为 `add_custom_command + file(GLOB CONFIGURE_DEPENDS)` 触碰任意 `*.comp` 自动重嵌入 + relink `vulkan_backend.cpp` (`libburstmerge/CMakeLists.txt:38-83`).
	- **Dense alignment GPU 时间随 `tile_size²` 增长 (已修复)**: `dense_level.comp` v3 设计为 "每 workgroup(8×8) 处理 64 个 tile, 每 thread 1 tile, 内层 ts² px 串行 + 4 候选 Kahan M=8 累加". 大 tile (96, 128) 时 64 个 float 累加器占用满 register file, SM occupancy 暴跌, 性能呈平方劣化. SLg2 48 帧 5060 Ti 实测 dense 时间 (v3): t=16: 7.13s / t=32: 9.17s / t=48: 10.30s / t=64: 13.55s / t=96: 19.42s / t=128: 29.14s. 同区间 CPU 几乎平 (15.2→17.8s) 因 OpenMP 把 ts² 平摊到 ~16 核. → **用户案例 `--backend vulkan -a dense -t 96 -m expbkt-avg` (5060 Ti): GPU v3 19.79s vs CPU 15.81s, GPU 反而慢 25%**. 修复为 `dense_level.comp` v4 (2026-07-04): **每 workgroup(8×8) 协同处理 1 个 tile (64 thread per tile)**, 共享 shared memory 做 64-thread partial cost 的 tree-reduction; inner loop ts² 工作量按 stride 64 平摊到 8 个 Kahan 累加器. CPU dispatch 维度从 `(ceil(tiles_x/8), ceil(tiles_y/8))` 变 `(tiles_x, tiles_y)`. 修复后 (v4) 性能: t=16: 6.7s / t=32: 6.7s / t=48: 6.8s / t=64: 6.5s / t=96: 7.4s / t=128: 7.4s — **ts² scaling 消除, 全 tile 大小近常数**. 用户案例现 GPU 8.09s vs CPU 16.27s, **GPU 快 2.0×**. 输出与 CPU 仍近像素级一致 (LongUnder2 dense t=96 包围曝光 + 高光恢复: MAD 0.022, 16-bit 量级单 LSB 抖动). **fp64 v4 (2026-07-05) 同步采用 64-thread-per-tile 协同设计, 移除 Kahan (double 精度本身足够). 真实 v3 vs v4 公平对比 (RTX 3080, LongUnder2 16帧, dense 对齐): v3 GPU 21.31s vs v4 GPU 11.22s vs CPU 21.32s — v4 比 v3 快 1.9×, 比 CPU 快 1.9×. ts² scaling 消除: v3 随 tile size 增长 (t=32:14.0s → t=128:29.6s, +111%), v4 近乎持平 (t=32:11.2s → t=128:12.4s, +11%). 包围曝光精度: ExBkt t=128 v4 fp64 对 CPU bit-identical (max 0px, MAD 0); LongUnder2 t=96 MAD 0.0037, MAXDIFF 2.

## 尚未实现 / 占位
	- Vulkan 后端 **已实现** (见上方 "Vulkan GPU 后端" 章节). 全部合并模式 (spatial/linear/temporal/freq-laplacian/wiener-fft) 与 CPU 近像素级一致 (等曝光 MAD < 0.02%; 包围曝光 < 0.01%); 对齐 standard+dense bit-identical (max 0px). **全部使用 float (shaderFloat64 默认不启用); `dense_level.comp` 使用 strided M=8 Kahan 累加 + 64-thread-per-tile 协同 + FMA 减少 float 舍入误差; `BURSTMERGE_GPU_FP64=ON` 时 `dense_level_fp64.comp` 的 `tileCostThreadFp64` 使用朴素 double 累加 (v4 自 2026-07-05 起与 fp32 v4 同 64-thread-per-tile 协同, 无 Kahan), shaderFloat64 按需启用**. WienerFftRobust 在 GPU 路径未实现 (会抛异常拒绝); hot-pixel repair 尚未在 GPU 路径实现 (当前跳过). [新增] 高光恢复 (highlight recovery) 在 GPU 路径已实现 (Bayer-only, `highlight_recovery.comp`, `prepare_texture` 之后 dispatch). [新增] 曝光加权合并 (spatial/freq-laplacian/freq-wiener) 与链式对齐 (transmission) 均已在 GPU 实现. `IComputeBackend`/`SubGraph` 通用节点图接口保留但未被 Vulkan 后端使用.
...
==============================
	...
	- [新增] GPU fp64 dense 对齐: `libburstmerge/src/compute/vulkan/shaders/dense_level_fp64.comp` (唯一使用 `double` 的着色器, `tileCostThreadFp64` 累加); `vulkan_backend.cpp` (`shaderFloat64` 条件启用 + `has_fp64` flag); `gpu_pipeline.cpp` `DenseAlignGPU` (shader 选择 `dense_level_fp64` vs `dense_level`, dispatch `(tx_L, ty_L, 1, local 8x8)` fp32/fp64 共用); CMake 开关 `BURSTMERGE_GPU_FP64` (默认 OFF); `embed_shaders.ps1` (`-IncludeFp64` 参数控制是否编译 `dense_level_fp64.comp`). **v4 (2026-07-05) 同 fp32 v4 协同设计**
	- [新增] GPU fp32 strided 累加: `libburstmerge/src/compute/vulkan/shaders/dense_level.comp` `tileCostThread` (v3 时名为 `tileCost`) 使用 M=8 独立 float 累加器 (pairwise merge) + Kahan compensation (SSD 模式) + stepped 内层循环 (避免动态索引致寄存器 spill); 超参数 `kStrideM` (默认 8, 可改为 1 回退为顺序累加); 需 cmake 重新 configure 以重新生成 SPIR-V. **v4 (2026-07-04)** 改为 64-thread-per-tile 协同设计 (workgroup 8×8 = 64 thread 协同处理 1 tile, shared memory tree-reduction 累加 partial costs); dispatch 维度变 `(tiles_x, tiles_y, 1)` 替代 v3 的 `(ceil(tiles_x/8), ceil(tiles_y/8))`. ts² scaling 消除, t=128 从 29.14s 降至 7.40s (4× 加速).
