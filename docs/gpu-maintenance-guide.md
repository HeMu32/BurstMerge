...
例外: `BURSTMERGE_GPU_FP64=ON` 时编译 `dense_level_fp64.comp` (double 累加, `shaderFloat64` 按需启用), 仅供极端包围曝光 dense 对齐使用。默认 fp32 路径 `dense_level.comp` v4 使用 strided M=8 Kahan 累加器 (per-thread pairwise merge) + 64-thread-per-tile 共享内存 tree-reduction, 仍是纯 float, 不违反此约束。fp64 v4 (2026-07-05) 同样是 64-thread-per-tile 协同设计, 仅 `tileCostThreadFp64` 用朴素 double 累加 (无 Kahan), 仍只在 dense 对齐路径使用 `double`。

> **2026-06-30 dense_level.comp v2 优化**: shader 从 52 次独立 `tileCost()` 调用 (每次独立遍历 tile 全部像素, 重复读取 ref buffer) 重写为 **fused candidate evaluation**: 候选按 `kCandGroup=4` 分批, 每批内 `ref[pixel]` 只读一次, 共享给批内所有候选; 仅 cmp 按候选位移分别读取。全局内存读取从 104 次/像素降至 69 次/像素 (~34%). 实测 RTX 3080 上 dense 对齐阶段加速 ~28% (0.43s→0.31s/帧), 输出与旧版 bit-identical (MAD=6.3e-05, MAXDIFF=1). Strided 累加精度不变.

> **2026-07-04 dense_level.comp v4 大 tile 性能修复**: v3 仍是 "1 thread per tile, 内层 ts² px 串行" 设计, 在 `--tile 96/128` 大 tile 时每 thread 累加 9216~16384 px × ncand, 8×8=64 个 float 累加器占用满 register file, SM occupancy 暴跌, GPU 时间随 tile_size² 增长 (SLg2 48 帧 5060 Ti: t=32 9.17s, t=96 19.42s, t=128 29.14s). 用户案例 `--backend vulkan -a dense -t 96 -m expbkt-avg` GPU 19.79s 比 CPU 15.81s 反而慢 25%. v4 改为 **64-thread-per-tile 协同设计**: 每个 workgroup(8×8) 处理 1 个 tile (而非 64 个), thread tid 处理 tile 内 stride-64 的 1/64 像素, 共享 shared memory (4*64 float = 1KB) 做 64-thread partial cost 的 pairwise tree-reduction. dispatch 维度变 `(tiles_x, tiles_y, 1)` 替代 `(ceil(tiles_x/8), ceil(tiles_y/8))`. ts² scaling 完全消除 (v4: t=32 6.7s, t=96 7.4s, t=128 7.4s); 用户案例 GPU 8.09s vs CPU 16.27s, GPU 快 2.0×. 输出与 CPU 仍近像素级一致 (LongUnder2 dense t=96 包围曝光 + 高光恢复: MAD 0.022, 16-bit 量级单 LSB 抖动). `dense_level_fp64.comp` 同步升级为 v4 (2026-07-05): 64-thread-per-tile 协同设计, 原生双精度 `cost += d*d` 累加, 共享 `float sh_cost[4*64]` 1KB + `reduceOneCand` 6 层树归约. Dispatch `(tx_L, ty_L, 1)` fp32/fp64 共用. 实测 RTX 3080 (LongUnder2 16帧, t=96): GPU 11.22s vs CPU 21.32s (GPU 1.9× 加速). 与旧 v3 fp64 公平对比 (正确 dispatch): v3 21.31s, v4 11.22s — 同场景下 v4 快 1.9×. ts² scaling 消除: v3 t=32 14.0s → t=128 29.6s (+111%); v4 t=32 11.2s → t=128 12.4s (+11%). 精度: LongUnder2 MAD 0.00374, MAXDIFF 2, 0.37%; ExBkt t=128 MAD 0 (完全 bit-identical).
