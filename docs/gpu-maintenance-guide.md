...
例外: `BURSTMERGE_GPU_FP64=ON` 时编译 `dense_level_fp64.comp` (double 累加, `shaderFloat64` 按需启用), 仅供极端包围曝光 dense 对齐使用。默认 fp32 路径 `dense_level.comp` v4 使用 strided M=8 Kahan 累加器 (per-thread pairwise merge) + 64-thread-per-tile 共享内存 tree-reduction, 仍是纯 float, 不违反此约束。
